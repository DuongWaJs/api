exports.info = {
  ...
  example_url: [
    {
    method: 'GET',
    query: '?url=https://example.com/name.jpg',
    desc: 'tìm nguồn ảnh',
    }
  ],
};
exports.methods = {
  get: async(req, res, next)=> {
    ...
    module.exports = {
      ...
      run: async (req, resp) => {
        try {
          if (!req.query.url) return resp.json({
            error: "Vui lòng nhập link url"
          })
          ...
          for (let i = 0; i < res.data.results.length; i++) {
            ...
            data.push({
              thumbnail: img, url: url, title: title, member_name: member_name
            })
          }
          resp.json(data)
        }catch(e) {
          resp.json({
            error: "Không tìm thấy ảnh này !"
          })}
      }
    }
  },
